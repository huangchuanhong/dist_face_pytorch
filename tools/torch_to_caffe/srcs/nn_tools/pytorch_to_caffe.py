...
def _adaptive_avg_pool2d(raw, input, output_size=1):
    assert(output_size == 1 or output_size==(1, 1))
    x = raw(input, output_size)
    
    layer_name = log.add_layer(name='global_avg_pool')
    top_blobs = log.add_blobs([x], name='global_avg_pool_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='Pooling',
                                  bottom=[log.get_blobs(input)], top=top_blobs)
    layer.pool_param(type='AVE', global_pooling=True)
    log.cnet.add_layer(layer)
    return x
...
def _softmax(raw, input, dim=None, _stacklevel=3):
    ...
    print('log.get_blobs(input)={}'.format(log.get_blobs(input)))
...
for op_name in F.__dict__:
    if op_name in F_supported:
        ...
        #print('raw_func={}'.format(raw_func))
        transfer_func=globals()['_'+op_name]
        #print('transfer_func={}'.format(transfer_func))
...
def torch_add(raw,*args):
    ...
    bottom_ = [log.get_blobs(arg_) for arg_ in args]
    layer = caffe_net.Layer_param(name=layer_name, type='Eltwise',
                                  bottom=bottom_, top=top_blobs)
...
def torch_mul(raw, *args):
    ...
    if args[0].shape == args[1].shape:
        ...
        layer = caffe_net.Layer_param(name=layer_name, type='Eltwise',
                                      bottom=[log.get_blobs(args[0]), log.get_blobs(args[1])], top=top_blobs)
        log.cnet.add_layer(layer)
    elif args[0].shape[0] == args[1].shape[0] and args[0].shape[1] == args[1].shape[1] and args[1].shape[2] == args[1].shape[3] == 1:
        args1_reshape = args[1].reshape((args[1].shape[0], args[1].shape[1]))
        layer1_name = log.add_layer(name='reshape')
        args1_reshape_blobs = log.add_blobs([args1_reshape], name='reshape_blob') 
        layer1 = caffe_net.Layer_param(name=layer1_name, type='Reshape',
                                  bottom=[log.get_blobs(args[1])], top=args1_reshape_blobs)
        layer1.param.reshape_param.shape.CopyFrom(caffe_net.pb.BlobShape(dim=[0,0]))
        log.cnet.add_layer(layer1)
        layer2_name = log.add_layer(name='scale')
        top_blobs = log.add_blobs([x], name='scale_blob')
        layer2 = caffe_net.Layer_param(name=layer2_name, type='Scale',
                                       bottom=[log.get_blobs(args[0]), args1_reshape_blobs[0]], top=top_blobs)
        layer2.param.scale_param.axis = 0
        log.cnet.add_layer(layer2)
    return x
...
def torch_sigmoid(raw, *args):
    x = raw(*args)
    if not NET_INITTED:
        return x
    layer_name = log.add_layer(name='sigmoid')
    top_blobs = log.add_blobs([x], name='sigmoid_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='Sigmoid',
                                  bottom=[log.get_blobs(args[0])], top=top_blobs)
    log.cnet.add_layer(layer)
    return x

def torch_transpose(raw, *args):
    x = raw(*args)
    if not NET_INITTED:
        return x
    layer_name = log.add_layer(name='transpose')
    top_blobs = log.add_blobs([x], name='transpose_blob')
    layer = caffe_net.Layer_param(name=layer_name, type='Permute',
                                  bottom=[log.get_blobs(args[0])], top=top_blobs)
    order = list(range(args[0].dim()))
    order[args[1]] = args[2]
    order[args[2]] = args[1]
    layer.param.permute_param.order.extend(order)
    log.cnet.add_layer(layer)
    return x
...
def trans_net(net, input_var, name='TransferedPytorchModel'):
    ...
    for name, layer in net.named_modules():
        ...
        print(name)
    out = net(input_var)#, return_loss=False)
