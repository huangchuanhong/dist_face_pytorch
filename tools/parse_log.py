...
field_list = []#"iter", "loss", "accuracy", "loss_bbox", "loss_cls", "rpn_loss_bbox", "rpn_loss_cls"]
field_tobe_list = []#"accuracy", "loss_bbox", "loss_cls", "rpn_loss_bbox", "rpn_loss_cls"]
show_field_list = ["sp", "loss"]#, "loss_bbox", "loss_cls", "rpn_loss_bbox", "rpn_loss_cls"]
...
def parse_log(log):
    ...
    for index in part_log_indexlist:
        ...
        loss = partlog.split(', loss:')[1].split(",")[0]
        if "loss_at" in partlog:
            atloss = partlog.split("loss_at:")[1].split(",")[0]    	
            loss_test.append(float(loss)-float(atloss))
        else:
...
def show_log(names, losses, is_show):
    ...
    plt.ylim([0.0,300.0])#300.0])
    plt.xlabel("iter x *")
    #plt.show()
    plt.savefig('parse_log.jpg')
